...
// --- PENTING: Ganti token ini dengan token valid Anda ---
const BEARER_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MSwicm9sZSI6IkFETUlOX1BVU0FUIiwic3BidUlkIjoxLCJpYXQiOjE3NTgwOTMyMjcsImV4cCI6MTc1ODE3OTYyN30.Nrc4PxFeOnK6vvT_jZBM_gfGqkm1nqsISxL7BR5YMjw'; // Ganti dengan token yang valid
const API_URL = 'https://ptadisejahtera.duckdns.org/api/v1/admin/spbus';

// ====================================================================
//              TYPESCRIPT TYPES
// ====================================================================

// --- PERBAIKAN 1: Menambahkan definisi tipe 'Tab' ---
...
// ====================================================================
//            KOMPONEN: FINANCIAL REPORT CARD
// ====================================================================
...
// ====================================================================
//              KOMPONEN: SPBU DETAIL (DENGAN EKSPOR PDF LENGKAP)
// ====================================================================
...
const SPBUDetail: React.FC<SPBUDetailProps> = ({ spbu, onBack }) => {
  ...
  const handleExportDetailPDF = () => {
    ...
    const addTable = (title: string, head: string[][], body: any[][]) => {
      const estimatedHeight = 15 + (body.length * 7) + 10;
      if (startY + estimatedHeight > doc.internal.pageSize.height && startY > 50) {
        doc.addPage();
        startY = 20;
      }
    ...
    if (spbu.fuelSale?.length) {
      addTable('Laporan Keuangan', [['Tanggal', 'Shift', 'Liter', 'Total Harga']],
        spbu.fuelSale.map(s => [formatDate(s.tanggal), s.shift, `${s.jumlahLiter} L`, formatCurrency(parseFloat(s.totalHarga))])
      );
    }
    
    // --- PERBAIKAN 2: Menggunakan optional chaining (?.) ---
    if (spbu.tanks?.length) {
        addTable('Status Tangki BBM', [['Jenis BBM', 'Kapasitas (L)', 'Volume Saat Ini (L)', 'Persentase (%)']],
            spbu.tanks.map(tank => {
                const percentage = ((parseFloat(tank.current_volume) / parseFloat(tank.capacity)) * 100).toFixed(1);
                return [
                    tank.fuel_type,
                    tank.capacity,
                    tank.current_volume,
    ...
    if (spbu.equipmentDamageReport?.length) {
      addTable('Laporan Kerusakan', [['Tanggal', 'Unit', 'Deskripsi']],
        spbu.equipmentDamageReport.map(r => [formatDate(r.tanggalKerusakan), r.namaUnit, r.deskripsiKerusakan])
      );
    }

    if (spbu.issueReport?.length) {
      addTable('Laporan Masalah', [['Tanggal', 'Judul', 'Deskripsi']],
        spbu.issueReport.map(r => [formatDate(r.tanggal), r.judulLaporan, r.deskripsiLaporan])
      );
    }

    if (allChecklists.length > 0) {
      addTable('Log Checklist', [['Tanggal', 'Tipe', 'Aktivitas', 'Status']],
        allChecklists.map(item => [formatDate(item.tanggal), item.type, getChecklistDescription(item), item.checklistStatus || 'TERLAKSANA'])
      );
    }

    if (spbu.users?.length) {
      addTable('Data Karyawan', [['Nama', 'Jabatan']],
        spbu.users.map(user => [user.name, user.role])
      );
    }
    
    if (spbu.pumpUnit?.length) {
        addTable('Unit Pompa', [['Kode Pompa']],
            spbu.pumpUnit.map(p => [p.kodePompa])
        );
    }

    if (spbu.stockDelivery?.length) {
        addTable('Riwayat Pengiriman Stok', [['Tanggal Kirim', 'Produk & Volume']],
            spbu.stockDelivery.map(d => [
                formatDate(d.createdAt),
                Object.entries(d)
                    .filter(([key, value]) => key.startsWith('volume') && parseFloat(value as string) > 0)
                    .map(([key, value]) => `${key.replace('volume', '')}: ${value} L`)
                    .join(', ')
            ])
        );
    }
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
              ...
              {/* --- PERBAIKAN 2: Menggunakan optional chaining (?.) --- */}
              {spbu.tanks?.length > 0 ? spbu.tanks.map(tank => {
        ...
        return (
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
              ...
              {/* --- PERBAIKAN 2: Menggunakan optional chaining (?.) --- */}
              {spbu.equipmentDamageReport?.length > 0 ? spbu.equipmentDamageReport.map((report: any) => (
            ...
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
              ...
              {/* --- PERBAIKAN 2: Menggunakan optional chaining (?.) --- */}
              {spbu.issueReport?.length > 0 ? spbu.issueReport.map((report: any) => (
                <div key={report.id} className="py-3 border-b border-slate-200 last:border-b-0">
              ...
              <table className="w-full text-sm text-left text-slate-600">
                <thead className="bg-slate-100 text-slate-700 uppercase">
                  <tr>
                    ...
                    <th scope="col" className="px-4 py-3">Aktivitas / Keterangan</th>
                  ...
                  {allChecklists.length > 0 ? allChecklists.map((item: any, index: number) => (
                    <tr key={index} className="border-b hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3">
                        <span className="px-2 py-1 text-xs font-semibold text-slate-700 bg-slate-200 rounded-full">{item.type}</span>
                      </td>
        ...
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
              ...
              {/* --- PERBAIKAN 2: Menggunakan optional chaining (?.) --- */}
              {spbu.pumpUnit?.length > 0 ? ( <ul className="space-y-2"> {spbu.pumpUnit?.map((pump: any) => <li key={pump.id} className="font-semibold text-slate-700 bg-slate-100 p-2 rounded-md">{pump.kodePompa}</li> )} </ul> ) : <p className="text-center text-slate-500 py-4">Belum ada data unit pompa.</p>}
            </div>
            <div className="md:col-span-2 bg-white p-6 rounded-xl shadow-md border border-slate-200">
              ...
              {/* --- PERBAIKAN 2: Menggunakan optional chaining (?.) --- */}
              {spbu.stockDelivery?.length > 0 ? ( 
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left text-slate-600">
                    <thead className="bg-slate-100 text-slate-700 uppercase">
                      <tr>
                        <th scope="col" className="px-4 py-3">Tanggal Pengiriman</th>
                        <th scope="col" className="px-4 py-3">Produk & Volume (Liter)</th>
                      </tr>
                    </thead>
                    <tbody>
                      {/* --- PERBAIKAN 2: Menggunakan optional chaining (?.) --- */}
                      {spbu.stockDelivery?.map((delivery: any) => ( 
                        <tr key={delivery.id} className="border-b hover:bg-slate-50">
                          ...
                          <td className="px-4 py-3 font-medium text-slate-800"> {Object.entries(delivery).filter(([key, value]) => key.startsWith('volume') && parseFloat(value as string) > 0).map(([key, value]) => `${key.replace('volume', '')}: ${value} L`).join(', ')} </td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
...
// ====================================================================
//              KOMPONEN: SPBU LIST (DENGAN EKSPOR PDF)
// ====================================================================
...
const SPBUList: React.FC<SPBUListProps> = ({ spbus, onSelectSpbu }) => {
  const calculateTotalSales = (fuelSales?: SPBU['fuelSale']): number => {
    if (!fuelSales || fuelSales.length === 0) return 0;
    return fuelSales.reduce((sum, sale) => sum + parseFloat(sale.totalHarga), 0);
  };
  ...
  const handleExportAllPDF = () => {
    ...
    autoTable(doc, {
        head: [['Kode SPBU', 'Alamat', 'Jml Karyawan', 'Jml Tangki', 'Total Penjualan']],
        body: spbus.map(spbu => [
            spbu.code_spbu,
            spbu.address,
            spbu.users?.length || 0,
            spbu.tanks?.length || 0,
            formatCurrency(calculateTotalSales(spbu.fuelSale))
        ]),
        startY: 35,
        theme: 'striped',
            ...
            <div className="flex-grow">
              ...
              <div className="mt-4 space-y-3 text-sm">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="font-semibold text-slate-800">{spbu.users?.length || 0} Karyawan</span>
                </div>
                <div className="flex items-center space-x-3">
                  <Fuel size={18} className="text-slate-500" />
                  <span className="font-semibold text-slate-800">{spbu.tanks?.length || 0} Tangki Penyimpanan</span>
                </div>
              </div>
            </div>
...
export default function MonitoringPage() {
  ...
  if (loading) {
    return <main className="bg-slate-50 min-h-screen"></main>;
  }
    ...
    <main className="bg-slate-50 min-h-screen p-4 sm:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        {selectedSpbu ? (
          ...
          <SPBUList spbus={spbus} onSelectSpbu={setSelectedSpbu} />
        )}
      </div>
    </main>
  );
