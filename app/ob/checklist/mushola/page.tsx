...
export default function ChecklistMusholaPage() {
  ...
  const renderFormFields = () => (
    <>
      ...
      <div>
        ...
        <Select value={form.aktifitasMushola || ""} onValueChange={(v) => setField("aktifitasMushola", v)}>
          ...
          <SelectContent>
            ...
            <SelectItem value="PERIKSA_FUNGSI_AC_PINTU_LAMPU_DAN_JENDELA">PERIKSA FUNGSI AC PINTU LAMPU DAN JENDELA</SelectItem>
            <SelectItem value="SEMROTKAN_PENGISI_RUANGAN">SEMPROTKAN PENGISI RUANGAN</SelectItem>
            <SelectItem value="SAPU_LANTAI_DAERAH_MUSHOLA">SAPU LANTAI DAERAH MUSHOLA</SelectItem>
            <SelectItem value="SAPU_DAN_LAP_DAERAH_PENYIMPANAN_SEPATU">SAPU DAN LAP DAERAH PENYIMPANAN SEPATU</SelectItem>
  ...
  // --- TAMBAHAN BARU: Fungsi Ekspor PDF untuk Mushola ---
  const handleExportPDF = () => {
    ...
    doc.text("CHECKLIST KEGIATAN HOUSEKEEPING MUSHOLA", doc.internal.pageSize.getWidth() / 2, 15, { align: "center" });
    ...
    const activities = [
        ...
        { value: "PERIKSA_FUNGSI_AC_PINTU_LAMPU_DAN_JENDELA", label: "Periksa fungsi AC, Pintu, Lampu & Jendela" },
        { value: "SEMROTKAN_PENGISI_RUANGAN", label: "Semprotkan pewangi ruangan" },
        { value: "SAPU_LANTAI_DAERAH_MUSHOLA", label: "Sapu lantai daerah mushola" },
        { value: "SAPU_DAN_LAP_DAERAH_PENYIMPANAN_SEPATU", label: "Sapu & lap daerah penyimpanan sepatu" },
    ...
    const body = activities.map(activity => {
      ...
      const checksForActivity = checklists.filter(c => c.aktifitasMushola === activity.value);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardHeader><CardTitle>Data Checklist Mushola</CardTitle></CardHeader>
        <CardContent>
          {loading ? (<p>Loading...</p>) : checklists.length === 0 ? (<p>Belum ada data checklist mushola.</p>) : (
            <Table>
              ...
              <TableBody>
                {checklists.map((c) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>{c.shift}</TableCell><TableCell>{c.aktifitasMushola.replace(/_/g, " ")}</TableCell>
      ...
      {/* Modal Tambah */}
      {openAddModal && (<div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50 overflow-y-auto"><div className="bg-white rounded-2xl p-6 w-full max-w-3xl"><h2 className="text-xl font-semibold mb-4">Tambah Checklist Mushola</h2><div className="grid grid-cols-2 gap-4 max-h-[70vh] overflow-y-auto">{renderFormFields()}</div><div className="flex justify-end gap-2 mt-4"><Button variant="outline" onClick={() => setOpenAddModal(false)}>Batal</Button><Button onClick={handleAdd}>Simpan</Button></div></div></div>)}
      {/* Modal Edit */}
      {openEditModal && editing && (<div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50 overflow-y-auto"><div className="bg-white rounded-2xl p-6 w-full max-w-3xl"><h2 className="text-xl font-semibold mb-4">Edit Checklist Mushola</h2><div className="grid grid-cols-2 gap-4 max-h-[70vh] overflow-y-auto">{renderFormFields()}</div><div className="flex justify-end gap-2 mt-4"><Button variant="outline" onClick={() => {setOpenEditModal(false); setEditing(null);}}>Batal</Button><Button onClick={handleUpdate}>Simpan</Button></div></div></div>)}
    </div>
  );
}